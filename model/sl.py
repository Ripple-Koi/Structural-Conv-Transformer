...
for epoch in range(EPOCHS):
    ...
    for i in range(len(input_tensor_val) // BATCH_SIZE):
        ...
        pred = infer_step(input_tensor_val[i * BATCH_SIZE:(i + 1) * BATCH_SIZE, :, :],
                                  target_tensor_val[i * BATCH_SIZE:(i + 1) * BATCH_SIZE, :, 0:2])
...
error_last_step = final_position_error.eval(result_last_step, input_tensor_val[:, :, 0:2], groundtruth, FREQUENCY, num_vehicles)
