...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training,
        ...
        if training:
            ...
            # because we query only the last frame in each iteration (seq_len_q == 1).
            ...
            # attn1.shape == (batch_size, 1, d_model)
            attn1, attn_weights_block1 = self.mha1(x, x, x[:, -1:, :], look_ahead_mask[:, :, -1:, :])
            attn1 = self.dropout1(attn1, training=training)
            out1 = self.layernorm1(attn1 + x[:, -1:, :])  # (batch_size, 1, d_model)

            attn2, attn_weights_block2 = self.mha2(
                enc_output, enc_output, out1, padding_mask)  # (batch_size, 1, d_model)
            attn2 = self.dropout2(attn2, training=training)
            out2 = self.layernorm2(attn2 + out1)  # (batch_size, 1, d_model)

            ffn_output = self.ffn(out2)  # (batch_size, 1, d_model)
            ffn_output = self.dropout3(ffn_output, training=training)
            out3 = self.layernorm3(ffn_output + out2)  # (batch_size, 1, d_model)
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, cache, training,
        ...
        if training:
            ...
            for i in range(self.num_layers):
                # x.shape: (batch_size, target_seq_len, d_model) -> (batch_size, 1, d_model)
                x, block1, block2 = self.dec_layers[i](x, enc_output, training,
                                                       look_ahead_mask, padding_mask)
                ...
                if cache is not None:
                    # cache.shape == (num_layers, batch_size, target_seq_len - 1, d_model)
                    # x.shape: (batch_size, 1, d_model) -> (batch_size, target_seq_len, d_model)
            ...
            if cache is not None:
                # (num_layers, batch_size, (target_seq_len - 1) + 1, d_model)
            ...
            # in case of inferring 1st frame
                ...
                # so cache.shape[2] is target_seq_len - 1 (2 - 1)
                cache = tf.stack(layer_cache, axis=0)  # (num_layers, batch_size, 1, d_model)
...
def create_masks(inp, tar):
    ...
    # however in inferring target one by one, broadcast: (batch_size, num_heads, 1, inp_seq_len)
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inputs, cache, training):
        ...
        if enc_output_cache is None:
            enc_output_cache = self.encoder(inp, training, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)

        # dec_output.shape == (batch_size, tar_seq_len, d_model)
        dec_output, attention_weights, dec_cache = self.decoder(
            tar, enc_output_cache, dec_cache, training, look_ahead_mask, dec_padding_mask)
...
def error_function(real, pred):
    error_lat = abs(real[:, :, 4] - pred[:, :, 4])  # (batch_size, tar_seq_len)
    error_long = abs(real[:, :, 3] - pred[:, :, 3])  # (batch_size, tar_seq_len)
    ...
    error_lat *= mask  # (batch_size, tar_seq_len)
    error_long *= mask  # (batch_size, tar_seq_len)
...
def train_step(inp, tar):
    tar_inp = tf.concat([inp[:, -1:, :], tar[:, :-1, :]], axis=1)
    tar_real = tar[:, :, :]

    with tf.GradientTape() as tape:
        predictions, _, _ = transformer([inp, tar_inp], [None, None], training=True)
...
def infer_step(inp, tar):
    # the last frame of inp (current trajectory), shape: (batch size, 1, feature size)
    tar_inp = inp[:, -1:, :]
    tar_real = tar[:, :, :]
    cache = [None, None]
    for i in range(tar_real.shape[1]):
        predictions, _, cache = transformer([inp, tar_inp], cache, training=False)

        # select the last frame from the tar_seq_len dimension
        last = predictions[:, -1:, :]  # (batch_size, 1, feature size)
        ...
        # tf.debugging.assert_equal(tf.cast(tar_inp[:, 1:, :], tf.float16),
        #                           tf.cast(predictions, tf.float16), summarize=300)

    error_lat, error_long = error_function(tar_real, tar_inp[:, 1:, :])
    train_error_lat(error_lat)
    train_error_long(error_long)
    # all_steps, last_step
    return tar_inp[:, 1:, :], predictions  # (batch_size, seq_len, feature_size)
...
def split(data):
    ...
    input_tensor = data[:, :6 * FREQUENCY, :]
    target_tensor = data[:, -5 * FREQUENCY:, :]
