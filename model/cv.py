...
from utils import evaluation, interpolate_pred
...
def juggle_and_split(data):
    ...
    data = np.reshape(
        data[:, :: int(10 / FREQUENCY), :, :], (-1, 11 * FREQUENCY * num_vehicles, 5)
    )
...
graph = np.stack(
    [
        np.load(data_dir)["GLOBAL"],
        np.load(data_dir)["MEDIUM"],
        np.load(data_dir)["LOCAL"],
    ],
    axis=1,
)
traj = np.load(data_dir)["ORIGIN"][:, :, 3:5].astype("float32")
...
error_last_step = evaluation(result_last_step, target_traj)
