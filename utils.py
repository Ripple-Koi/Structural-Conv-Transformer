...
class CustomSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    def __init__(self, peak_lr=0.001, end_lr=0.0001, total_steps=3000):  # total training steps around 70000
...
def evaluation(interp_pred, groundtruth):
    ...
    average_long_error = np.stack([np.mean(abs(delta_d[:, 0:10, 0]), axis=1),
                                   ...
                                   np.mean(abs(delta_d[:, 0:60, 0]), axis=1)], axis=1)  # [N, 6]
    average_lat_error = np.stack([np.mean(abs(delta_d[:, 0:10, 1]), axis=1),
                                  ...
                                  np.mean(abs(delta_d[:, 0:60, 1]), axis=1)], axis=1)  # [N, 6]
    average_distance_error = np.stack([np.mean(v_sum(delta_d[:, 0:10, :]), axis=1),
                                       ...
                                       np.mean(v_sum(delta_d[:, 0:60, :]), axis=1)], axis=1)  # [N, 6]
    return np.stack([np.mean(final_long_error, axis=0),
                     ...
                     np.mean(average_distance_error, axis=0)], axis=0)  # [6, 6]
