...
from utils import CustomSchedule, evaluation, loss_function, error_function, juggle_and_split, interpolate_pred
...
predictor = structural_conv_transformer(num_layers=1, 
                                        d_model=32, 
                                        num_heads=4, 
                                        dff=32, 
...
if LR_SCHEDULE:
    learning_rate = CustomSchedule(peak_lr=LR, end_lr=LR / 5, total_steps=EPOCHS * TRAIN_STEPS)
else:
    learning_rate = LR
...
for epoch in range(EPOCHS):
    ...
    for step in range(TRAIN_STEPS):
        ...
        train_step(input_traj_train[step * BATCH_SIZE:(step + 1) * BATCH_SIZE, ::int(10 / FREQUENCY), :],
                   target_traj_train[step * BATCH_SIZE:(step + 1) * BATCH_SIZE, ::int(10 / FREQUENCY) * NUM_VEHICLES, :],
    ...
    for step in range(TEST_STEPS):  # Infer in batch to avoid OOM
        ...
        pred = infer_step(input_traj_test[step * BATCH_SIZE:(step + 1) * BATCH_SIZE, ::int(10 / FREQUENCY), :],
                          target_traj_test[step * BATCH_SIZE:(step + 1) * BATCH_SIZE, ::int(10 / FREQUENCY) * NUM_VEHICLES, :],
                          graph_test[step * BATCH_SIZE:(step + 1) * BATCH_SIZE, :, :, :, :])
        
        interp_pred = interpolate_pred(pred, input_traj_test[step * BATCH_SIZE:(step + 1) * BATCH_SIZE, -1:, :])
        batch_metrics = evaluation(interp_pred, target_traj_test[step * BATCH_SIZE:(step + 1) * BATCH_SIZE, ::NUM_VEHICLES, :])
    ...
    print(f'Epoch {epoch + 1} Long Error {np.mean(np.array(long_error)):.4f} Lat Error {np.mean(np.array(lat_error)):.4f}')
    with np.printoptions(formatter={'float': '{: 0.3f}'.format}):
        print(np.mean(np.array(metrics), axis=0))

np.savetxt(result_path / 'metrics.csv', np.mean(np.array(metrics), axis=0), delimiter=',')
